        ...
        <div class="flex-none w-full max-w-full px-3">
            <div class="relative flex flex-col min-w-0 p-6 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-soft-xl rounded-2xl bg-clip-border">
                <div class="font-semibold border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
                    <h6>Tambah Stok Barang Baru</h6>
                </div>
                <div class="flex-auto ">
                    <form action="{{ route('admin.stok_barang.cluster.store') }}" method="POST" enctype="multipart/form-data" role="form">
                        @csrf
                        ...
                        <div class="mb-4">
                            <input type="number" min=0 oninput="validity.valid||(value='');" class="flex-auto focus:shadow-soft-primary-outline text-sm leading-5.6 ease-soft block appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 transition-all focus:border-red-300 focus:outline-none focus:transition-shadow max-w-full min-w-12" id="inputHistoriBarang" placeholder="Masukkan data stok" aria-label="Histori_barang" aria-describedby="Histori_barang-addon" autocomplete="off" value="{{ old('stok')}}" name="stok" required>
                            @error('stok')
