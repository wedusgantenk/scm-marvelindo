            ...
            <div class="relative flex flex-col min-w-0 p-6 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-soft-xl rounded-2xl bg-clip-border">
                ...
                <div class="flex-auto ">
                    <form id="barang" action="{{ route('admin.barang.update', $data['id']) }}" method="POST" enctype="multipart/form-data" role="form">
                        ...
                        <div class="mb-4">
                            <input type="text" class="flex-auto focus:shadow-soft-primary-outline text-sm leading-5.6 ease-soft block appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 transition-all focus:border-red-300 focus:outline-none focus:transition-shadow max-w-full min-w-12" id="inputBarang" placeholder="Masukkan nama barang" aria-label="barang" aria-describedby="barang-addon" autocomplete="off" value="{{ $data['nama'] }}" name="nama" required>
                            @error('nama')
                            <span id="Barang-addon" class="invalid-feedback" role="alert"></span>
                            @enderror
                        </div>
                        ...
                        <div class="mt-4">
                            <input id="fisik" name="fisik" value="1" class="w-5 h-5 text-red-600 rounded-md" type="checkbox" <?php if ($data['fisik'] == 1) echo 'checked'; ?> />
