                        ...
                        <table id="datatables" class="table max-w-full overflow-auto table-flush text-slate-800">
                            ...
                            <tbody class="px-6 py-3 text-xs font-semibold text-left border-spacing-4">
                                ...
                                <tr>
                                    ...
                                    <td class="text-sm font-normal leading-normal border-t"> {{ $d['username'] }} </td>
                                    <td class="flex text-sm font-normal leading-normal border-t">
                                        <a href="{{ route('admin.petugas.edit', $d['id']) }}" class="mx-2 btn btn-success btn-xs"><i class="fas fa-edit fa-lg hover:scale-102"></i></a>
                                        <a href="{{ route('admin.petugas.delete', $d['id']) }}" class="mx-1 btn btn-danger btn-xs">
                                            <form action="{{ route('admin.petugas.delete', $d['id']) }}" method="post" onsubmit="return confirm('Apakah yakin ingin menghapus petugas {{ $d['username'] }} ?')">
                                                @csrf
                                                @method('delete')
