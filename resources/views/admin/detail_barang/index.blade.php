                        ...
                        <table id="datatables" class="items-center w-full px-6 py-3 mb-0 align-top border-transparent border-gray-200 table-auto text-slate-600">
                            ...
                            <tbody class="px-6 py-3 text-xs font-semibold text-left border-spacing-4">
                                ...
                                <tr>
                                    ...
                                    <td class="text-sm font-normal leading-normal border-t"> {{ $d->barang_masuk->no_po ?? 'N/A' }} </td>
                                    <td class="text-sm font-normal leading-normal border-t"> {{ $d->kode_unik }} </td>
                                    <td class="text-sm font-normal leading-normal border-t">
                                        {{ $d['status'] == 0 ?'Tidak aktif' : ($d['status'] == 1 ?'Aktif' : $d['status'] ) }}
                                    </td>
                                    <td class="flex text-sm font-normal leading-normal border-t">
                                        <a href="{{ route('admin.detail_barang.edit', $d['id']) }}" class="mx-2 btn btn-success btn-xs"><i class="fas fa-edit fa-lg hover:scale-102"></i></a>
                                        <a class="mx-1 btn btn-danger btn-xs">
                                            <form action="{{ route('admin.detail_barang.delete', $d['id']) }}" method="post" onsubmit="return confirm('Apakah yakin ingin menghapus detail barang {{ $d->barang->nama }} ?')">
                                                @csrf
                                                @method('delete')
